...
![ezgif com-video-to-gif](https://github.com/JULIUSJIANG/u3d_ball_effect/assets/33363444/d3dc0b57-debd-4b2a-a896-e8d4787e2061)

## 简介

* 在用 Unity 的 Particle System 的时候，经常需要用一个 Billboard 模式的片去实现一个球状特效（能量球、冲击波、爆闪等），如下图所示：
...
* 通过 Billboard，仅用 4 个顶点即可实现球状效果，对比使用球状网格，这样大大降低了 Cpu、Gpu 的消耗。但这种实现方式在高度不够的时候容易发生切片，如下图所示：
...
* 该解决方案通过顶点着色器控制网格的 6 个顶点的世界坐标、纹理采样坐标来解决上述提到的切边问题，最终效果如下图所示：
...
* Particle System 引用的材质中的着色器换成 Jiang/ParticleSystemFoldAdd（项目中需要有 shader_particle_system_fold.shader），Texture 设置为效果纹理，GroundY 设置为地面的 Y 坐标（默认为 0），如下图所示：
...
* Particle System 的 Render Mode 改为 Mesh，如下图所示：
...
* 把网格设置成 ball_effect（项目中需要有 fbx_ball_effect.fbx），如下图所示：
...
* 勾选 Custom Vertex Streams，如下图所示：
...
* 添加 Tangent、Center，注意次序不能乱，如下图所示：
...
fbx_ball_effect.fbx 中的网格 ball_effect 是一个自带折痕的片，网格数据中折痕与底部重合，所以外观上没看出来，如下图所示：
...
* 目前该解决方案已应用到个人制作的各个特效中，效果稳定可靠。

* 本代码仓库长期维护，在使用上遇到问题或者有任何疑问，欢迎发送邮件到 2662774600@qq.com 进行反馈

* 如果对您有所帮助，希望你能给我个 Star！
